...
def get_convolutional_autoencoder_tamila_deep(data, config):
    ...
    x = Conv1D(ceil(filter_multiplier * 10), 3, activation=activation)(x)
    x = AveragePooling1D(2)(x)
    x = Conv1D(ceil(filter_multiplier * 8), 3, activation=activation)(x)
    x = AveragePooling1D(2)(x)
    x = Conv1D(ceil(filter_multiplier * 6), 3, activation=activation)(x)
    x = AveragePooling1D(2)(x)
    x = Conv1D(ceil(filter_multiplier * 4), 3, activation=activation)(x)
    x = AveragePooling1D(2)(x)
    x = Conv1D(ceil(filter_multiplier * 2), 3, activation=activation)(x)
    x = AveragePooling1D(2)(x)
    x = Conv1D(ceil(filter_multiplier * 1), 3, activation=activation)(x)
